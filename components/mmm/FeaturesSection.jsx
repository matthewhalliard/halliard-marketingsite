        ...
        <div className="mt-16 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-2">
          {featureCategories.map((category) => (
            <div key={category.title} className="rounded-2xl border border-slate-200 p-8">
              <h3 className="text-lg font-semibold text-[#1F1F1F] mb-6">{category.title}</h3>
              <ul className="space-y-4">
                {category.features.map((feature, index) => {
                  const [title, description] = feature.split(' - ')
                  return (
                    <li key={index} className="flex items-start">
                      <svg className="h-6 w-6 text-[#1F3FFF] mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                      </svg>
                      <div>
                        <span className="font-medium text-[#1F1F1F]">{title}</span>
                        {description && (
                          <span className="text-[#4B4B4B]"> - {description}</span>
                        )}
                      </div>
                    </li>
                  )
